...
void main() {
  ...
  group('迁移保真:默认值 / 注释 / 索引', () {
    ...
    test('IDENTITY 列 → IDENTITY;setval 复位序列', () {
      const cols = [
        DbMigrationColumn(
          name: 'id',
          sourceType: 'bigint',
          ...
          nullable: false,
          isPrimaryKey: true,
          isIdentity: true, // 现代 IDENTITY 列(无 nextval 默认)
        ),
      ];
      final ddl = DbMigration.buildCreateTable(
        DbEngine.postgres,
      ...
      final resets = DbMigration.buildResetSequenceStatements(
        DbEngine.postgres,
        DbEngine.postgres,
        cols,
        targetTable: 't',
      ...
      expect(resets.single, contains('setval(pg_get_serial_sequence'));
      expect(resets.single, contains('\'"app"."t"\''));
      expect(resets.single, contains('max("id")'));
      // 非自增列不产生 setval
      expect(
