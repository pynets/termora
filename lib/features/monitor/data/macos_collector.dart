...
class MacosCollector extends PlatformCollector {
  ...
  Future<MonitorSnapshot> sample() async {
    ...
    final results = await Future.wait([
      runForStdout('vm_stat', const []),
      runForStdout('netstat', const ['-ibn']),
      runForStdout('ps', const [
        'axo',
        'pid=,ppid=,user=,pcpu=,pmem=,rss=,state=,comm=',
      ]),
      runForStdout('sysctl', const [
        '-n',
        'hw.memsize',
        'vm.swapusage',
        'vm.loadavg',
      ...
      runForStdout('iostat', const ['-Id']),
      runForStdout('ioreg', const ['-rc', 'IOAccelerator', '-d', '1']),
      if (refreshSlow) ...[
    ...
    final vmStat = results[0];
    final netstat = results[1];
    final ps = results[2];
    final sysctl = results[3];
    final iostat = results[4];
    final ioregGpu = results[5];
    if (refreshSlow) {
      if (results[6] != null) _disks = parseDfOutput(results[6]!);
      _parseBattery(results.length > 7 ? results[7] : null,
          results.length > 8 ? results[8] : null);
    }

    final processes = ps == null ? const <ProcInfo>[] : parsePsOutput(ps);
  ...
  void _parseBattery(String? pmset, String? ioreg) {
    ...
    final percent = double.tryParse(
          RegExp(r'(\d+)%').firstMatch(pmset)?.group(1) ?? '',
        ) ??
        0;
...
typedef _VmDeallocateNative = Int32 Function(Uint32 task, IntPtr addr, IntPtr size);
