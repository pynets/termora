...
class DbMigrationColumn {
  ...
  /// 源引擎的默认值表达式(pg pg_get_expr / ch default_expression / sqlite dflt_value)
...
class DbMigration {
  ...
  static List<String> buildCreateTable(
    ...
    String? schema,
    String? tableComment,
    List<DbConstraintInfo> constraints = const [],
  }) {
    ...
    for (final c in columns) {
      ...
      // 生成列:表达式带列引用,绝不能当 DEFAULT(0A000);
      // pg→pg 重建生成性,其余场景降级为带数据的普通列
      final generatedPg =
          c.isGenerated &&
      ...
      final defaultSql = defaultExpr == null ? '' : ' DEFAULT $defaultExpr';
      ...
      if (target == DbEngine.clickhouse) {
        ...
        final wrapped =
            ...
            : type;
        // CH 列注释内联
        final comment = (c.comment == null || c.comment!.isEmpty)
            ? ''
            : ' COMMENT ${_string(target, c.comment!)}';
        defs.add('  ${ident(target, c.name)} $wrapped$defaultSql$comment');
      } else {
        ...
        final generatedSql = generatedPg
            ...
            : '';
        defs.add(
          '  ${ident(target, c.name)} $type$notNull$defaultSql'
          '$identitySql$generatedSql',
        );
      }
    }
    ...
    if (target == DbEngine.clickhouse) {
      ...
      if (tableComment != null && tableComment.isNotEmpty) {
        tail += ' COMMENT ${_string(target, tableComment)}';
      }
    } else {
      if (pk.isNotEmpty) {
        defs.add(
      ...
      // sqlite 不支持事后 ADD CONSTRAINT,外键内联进建表
      // (sqlite 的 definition 由 pragma 重建,已是本方言)
      if (source == DbEngine.sqlite && target == DbEngine.sqlite) {
        for (final c in constraints) {
          if (c.type == DbConstraintType.foreignKey) {
            defs.add('  ${c.definition}');
          }
        }
      }
      tail = ')';
    ...
    return [
      ...
      if (target == DbEngine.postgres) ...[
        if (tableComment != null && tableComment.isNotEmpty)
          'COMMENT ON TABLE $t IS ${_string(target, tableComment)}',
        for (final c in columns)
          if (c.comment != null && c.comment!.isNotEmpty)
            'COMMENT ON COLUMN $t.${ident(target, c.name)} '
                'IS ${_string(target, c.comment!)}',
      ],
    ];
  }

  /// 是否 pg serial 类整型列(默认值为 nextval('…'))
  static bool _isSerialDefault(DbEngine source, DbMigrationColumn c) {
    if (source != DbEngine.postgres) return false;
    final def = c.defaultValue?.trimLeft().toLowerCase() ?? '';
  ...
  /// 同引擎:原样(pg 的 nextval 由 IDENTITY 分支接管,这里排除);
  /// 跨引擎:仅可移植的字面量(数字/字符串/布尔)和当前时间函数。
  static String? _defaultExprFor(
    DbEngine source,
    DbEngine target,
    DbMigrationColumn c,
  ) {
    final def = c.defaultValue?.trim();
    if (def == null || def.isEmpty) return null;
    final lower = def.toLowerCase();

    if (source == target) {
      // pg 的 nextval 默认引用源库序列,目标端不存在 → 不原样带
      if (source == DbEngine.postgres && lower.startsWith('nextval(')) {
        return null;
      }
      return def;
    }
    ...
    // 跨引擎:数字
    if (RegExp(r'^-?\d+(\.\d+)?$').hasMatch(def)) return def;
    // 布尔
    ...
    // 带可选 ::cast 的字符串字面量('x' / 'x'::text)
    final str = RegExp(r"^('(?:[^']|'')*')(::[A-Za-z_ ]+)?$").firstMatch(def);
    if (str != null) return str.group(1);
    // 当前时间
    if (lower == 'now()' ||
        lower == 'current_timestamp' ||
        lower.startsWith('current_timestamp(')) {
      return target == DbEngine.sqlite ? 'CURRENT_TIMESTAMP' : 'now()';
    }
    return null; // 其他表达式方言不兼容,丢弃
  }

  /// 二级索引重放语句(建表后执行;失败不应中断迁移,调用方须容错):
  /// - pg 同引擎:重放 indexdef,替换 ON 到目标限定名,跳过主键索引,
  ///   加 IF NOT EXISTS;
  /// - sqlite 同引擎且表名未改:重放原 CREATE INDEX(加 IF NOT EXISTS);
  /// - CH(排序键已在 MergeTree 内)与跨引擎:不迁。
  static List<String> buildIndexStatements(
    ...
    if (source != target) return const [];
    final statements = <String>[];

    if (target == DbEngine.postgres) {
      final t = qualified(target, targetSchema, targetTable);
      for (final idx in indexes) {
        if (idx.name.endsWith('_pkey')) continue; // 主键随建表已建
        var def = idx.definition.trim();
        if (!def.toUpperCase().startsWith('CREATE')) continue;
        // ON <源限定名> USING → ON <目标限定名> USING
        def = def.replaceFirst(
          RegExp(r'\sON\s+.+?\sUSING\s', dotAll: true),
          ' ON $t USING ',
        );
        def = def
  ...
  /// 外键 / CHECK 约束语句(pg 同引擎;在**所有表建好、数据装完后**统一
  /// ALTER TABLE 追加 —— 一并解决建表顺序、循环引用、插入顺序问题)。
  /// 外键的 REFERENCES 目标重写:[preserveSchema] 时限定到源 schema,
  /// 否则剥掉 schema 落目标默认。跨引擎/CH 不迁;sqlite 走建表内联。
  static List<String> buildConstraintStatements(
    DbEngine source,
    DbEngine target,
    List<DbConstraintInfo> constraints, {
    required String targetTable,
    String? targetSchema,
    bool preserveSchema = false,
  }) {
    if (source != DbEngine.postgres || target != DbEngine.postgres) {
      return const [];
    }
    ...
    final statements = <String>[];
    for (final c in constraints) {
      var def = c.definition.trim();
      if (c.type == DbConstraintType.foreignKey && c.refTable != null) {
        // 把 REFERENCES 后的目标名(可能带源 schema 前缀/引号)换成目标端写法
        final refName = preserveSchema
            ? qualified(target, c.refSchema, c.refTable!)
            : ident(target, c.refTable!);
        def = def.replaceFirst(
          RegExp(r'REFERENCES\s+(?:"[^"]*"|[^\s(])+'),
          'REFERENCES $refName',
        );
      }
      statements.add(
        'ALTER TABLE $t ADD CONSTRAINT ${ident(target, c.name)} $def',
      );
    }
    return statements;
  }
